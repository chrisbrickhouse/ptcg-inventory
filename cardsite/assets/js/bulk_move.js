...
function loadTable(tableId, data) {
  ...
  $.each(Object.keys(data), function (index, card_id) {
    function makeInput(name, val, max_n) {
      var input =
        '<input class="form-control" style="width: 100%;" type="number" name="';
      input += name;
...
function getStashList(fromUUID, toUUID) {
  $.ajax({
    url:
      api_url +
      "?action=getBulkMoveTable&from_uuid=" +
      fromUUID +
      "&to_uuid=" +
      toUUID,
    type: "GET",
...
$(function () {
  function updateFromStashHeader() {
    $("#fromHeader").html(
      "Number in " + $fromStash.find("option:selected").text(),
    );
  }
  ...
  var $fromStash = $("#from-stash");
  var $toStash = $("#to-stash");
