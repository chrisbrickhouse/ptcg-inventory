...
function changeTable(sheetName) {
  $("table").replaceWith(
    $(XLSX.utils.sheet_to_html(workbook.Sheets[sheetName])).addClass(
      "table table-hover",
    ),
  );
...
function pushBulkChanges(dry_run = false) {
  ...
  var sheet = XLSX.utils.sheet_to_json(
    workbook.Sheets[$("#id_sheet_name").val()],
  );
  // Reset error latch
  error = false;
  if (
